...
mat = read.table(file.path(oncogrid.path, "oncoprint_mat.txt"),  header = TRUE, stringsAsFactors=FALSE, sep = "\t",check.names = FALSE)
mat[is.na(mat)] = ""
rownames(mat) = mat[, 1]
mat = mat[, -1]
mat = t(as.matrix(mat))
...
alter_fun = list(
  ...
  LOS = function(x, y, w, h) {
    grid.rect(x, y, w-unit(0.3, "mm"), h-unit(0.3, "mm"), gp = gpar(fill = "#39ac73", col = NA))
  },
  ...
  FSI = function(x, y, w, h) {
    grid.rect(x, y, w-unit(0.3, "mm"), h-unit(0.3, "mm"), gp = gpar(fill = "#008080", col = NA))
  },
  ...
  IFD = function(x, y, w, h) {
    grid.rect(x, y, w-unit(0.3, "mm"), h-unit(0.3, "mm"), gp = gpar(fill = "#3D2E8E", col = NA))
  },
  ...
  FUS = function(x, y, w, h) {
    grid.rect(x, y, w-unit(0.3, "mm"), h*0.66, gp = gpar(fill = "#cc0000", col = NA))
  }
)
col = c("GAI" = "#ff4d4d", "LOS" = "#39ac73" , "MIS" = "#77b300", "FUS" = "#cc0000","NOS" ="#80bfff", "FSD" = "#1a53ff", "FSI" ="#008080", "NOT" = "#9966ff","SPS" = "#E69F00","IFD" = "#3D2E8E","OVE" = "#dbc6eb","UNE" = "#709fb0")

# read annotation and TMB info
annot_info <- read.table(file.path(oncogrid.path, "annot_mat.txt"), header = TRUE, check.names = TRUE)
annot_info <- annot_info %>%
  column_to_rownames('Sample') %>%
  as.data.frame()
tmb_info <- read.table(file.path(oncogrid.path, "tmb_mat.txt"), header = TRUE, check.names = TRUE)
tmb_info <- tmb_info %>%
  column_to_rownames('Sample') %>%
  as.data.frame()

ha = HeatmapAnnotation(df = annot_info ,col = list(
  Sequencing_Experiment= c("WGS,WXS,RNA-Seq" = "#26734d","WGS,Panel,RNA-Seq" = "#1a3300","WGS" = "#009999","RNA-Seq" = "#bac964","WGS,RNA-Seq"="#5a3d55","WXS,RNA-Seq"="#96bb7c"),
  Cohort = c("CBTTC" = "#709fb0", "PNOC003" ="#a0c1b8","PNOC008" = "#f4ebc1"),
  Tumor_Descriptor = c("Primary" = "#d8b9c3","Progressive" = "#827397", "Initial_CNS_Tumor"= "#cee397","Progressive_Disease_Post_Mortem"="#4d4c7d","Recurrence" = "#363062","Second_Malignancy" = "#005082"),
  Integrated_Diagnosis = c("Brainstem_glioma_Diffuse_intrinsic_pontine_glioma" = "#e1f4f3","Diffuse_midline_glioma"="#706c61","Gliomatosis_Cerebri"="#6e5773","High_grade_glioma"="#347474"),
  OS_Status = c("DECEASED" ="#e9e2d0","LIVING" ="#7fcd91","NA" ="#f0efef" )),gp = gpar(col = "#595959"),simple_anno_size = unit(4, "mm"),annotation_name_side = "left",annotation_legend_param = list(
    Sequencing_Experiment = list(nrow =3),
    Cohort = list(nrow=3)
  ))

ha1 = HeatmapAnnotation(TMB = anno_barplot(tmb_info$TMB, 
...
png(filename = file.path(topDir, "complexHeatmap_oncogrid.png"), units = "in", width = 25, height = 14, res = 200) 
