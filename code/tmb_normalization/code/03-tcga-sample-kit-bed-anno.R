# This script uses all the downloaded bed files, calculate the length the bed files
...
bed_file_path = file.path(results_dir, 'bed_files', 'tcga_not_in_pbta')
...
bam_manifest <- read.delim(file.path(results_dir, "tcga_not_in_pbta_bam_manifest.tsv"))
...
bed_selection <- read.delim(file.path(results_dir, "tcga_not_in_pbta_bed_selected.tsv"), header = T, row.names=NULL, stringsAsFactors = F) %>%
...
readr::write_tsv(bam_manifest, file.path(results_dir, "tcga_not_in_pbta_bam_manifest_with_length.tsv"))
readr::write_tsv(bed_name_length_matched, file.path(results_dir, "tcga_not_in_pbta_unique_bed_with_length.tsv"))
...
# Read in the bed files
bed_file_path = file.path(results_dir, 'bed_files', 'tcga_in_pbta')
...
# Read in the manifest file and get a column that only contains the bed file (name Gh38 corrected)
bam_manifest <- read.delim(file.path(ref_dir, "pbta-tcga-manifest.tsv"))
...
bam_manifest <- bam_manifest %>% 
  dplyr::mutate(Tumor_Sample_Barcode = gsub(".*[.]TCGA", "TCGA", Tumor_BAM)) %>%
...
bam_manifest <- bam_manifest %>% 
  dplyr::mutate(bed_selected = dplyr::case_when(
...
# Write out the updated bam_manifest file
readr::write_tsv(bam_manifest, file.path(results_dir, "tcga_in_pbta_bam_manifest_with_length.tsv"))
readr::write_tsv(bed_name_length_matched, file.path(results_dir, "tcga_in_pbta_unique_bed_with_length.tsv"))
