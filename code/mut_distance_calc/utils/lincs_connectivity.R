...
lincs_connectivity <- function(input, num_sets = 25, method = c("LINCS", "Cor"), wtcs_fdr_cutoff = 0.05, trend_val = c("up", "down"), cor_score_cutoff = 0, output_dir){
  ...
  if(method == "LINCS"){
    ...
    upset = mapIds(org.Hs.eg.db, keys = cluster_upset$geneSymbol, column = "ENTREZID", keytype = "SYMBOL")
    downset = mapIds(org.Hs.eg.db, keys = cluster_downset$geneSymbol, column = "ENTREZID", keytype = "SYMBOL")
  ...
  fname <- file.path(output_dir, "poi_vtest_qSig_output.txt")
  ...
  if(length(drugs) != 0){
    ...
    fname <- file.path(output_dir, "poi_vtest_tsea_reactome_output.txt")
    ...
    fname <- file.path(output_dir, "poi_vtest_dsea_go_mf_output.txt")
    ...
    fname <- file.path(output_dir, "poi_vtest_dsea_go_mf_output.html")
    visSave(dnet_object, fname)
    fname <- file.path(output_dir, "poi_vtest_dsea_go_mf_output.pdf")
    ...
    p3 <- drug_barplots(dat = dsea_go_mf_df, 
                        ...
                        top = 20, fill_var = NULL,
                        title = "DSEA GO MF")
    title <- cowplot::ggdraw() + draw_label("POI v-test", fontface = 'bold')
    ...
    # set drug pathways barplot to NULL
    p <- NULL
  }
