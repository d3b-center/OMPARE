...
```{r read_data}
# read input files
source(file.path(utils_dir, "readData.R"))
...
source(file.path(code_dir, "driver.R"))
```
...
Transcriptomic Alterations (P2) {data-navmenu="Transcriptomic Alterations"}
===
Row {data-height=700}
---
...
```{r plot_pathway, fig.width = 28, fig.height = 10}
#Bar chart of genes
if(exists('expData')){
...
if(exists('expData')){
  p <- ImmuneProfile(fullmat = pbta.mat.all, 
                     fname = file.path(topDir, 'ImmuneScores', 'rawScores_pediatric.txt'))
  ggplotly(p)
}
```
...
if(exists('expData')){
  p <- ImmuneProfile(fullmat = tcga.gbm.mat, 
                     fname = file.path(topDir, 'ImmuneScores', 'rawScores_adult.txt'))
  ggplotly(p)
}
```
...
if(exists('expData')){
  p <- tisProfile(fname = file.path(topDir, 'ImmuneScores', 'tisScores.txt'), 
                  score = 'avg')
  ggplotly(p)
}
```
...
if(exists('expData')){
  p <- ImmuneProfile(fullmat = pbta.topCor, fname = file.path(topDir, 'ImmuneScores', 'topCor_rawScores.txt'))
  ggplotly(p)
}
```
...
gsPath <- tabulate_pathways(allCor = pbta.allCor, prefix = "pediatric_", comparison = "pediatric")
...
```{r ssgsea_hgg, fig.width = 28, fig.height = 10}
if(exists('expData')){
  p <- ssGSEA(topCor = pbta.mat.hgg, fname = file.path(topDir, 'GSVA', 'ssgsea_rawScores.txt'))
  ggplotly(p)
}
```
...
gsPath <- tabulate_pathways(allCor = tcga.gbm.allCor, prefix = "adult_", comparison = "adult")
datatable(gsPath[[1]], rownames=F, options = list(pageLength = 10))
```
...
if(exists('fusData')){
 knitr::include_graphics(file.path(topDir, "tmpRCircos.png")) 
}
```
...
fname <- file.path(topDir, 'complexHeatmap_phgg.png')
...
fname <- file.path(topDir, 'complexHeatmap_cgs.png')
knitr::include_graphics(fname) 
```
