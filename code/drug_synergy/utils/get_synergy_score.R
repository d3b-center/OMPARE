...
getSynScore2 <- function(tar1, tar2, sigNet1){
  ...
  sc1 <- closeness(sigNet1, vSet0)  
  sc2 <- betweenness(sigNet1, vSet0, directed=F) 
  sc3 <- page.rank(sigNet1, algo="prpack", vids=vSet0)$vector ### for normalization
  sc11 <- closeness(sigNet1, tar1)  
  sc12 <- betweenness(sigNet1, tar1, directed=F) 
  sc13 <- page.rank(sigNet1, algo="prpack", vids=tar1)$vector
  sc21 <- closeness(sigNet1, tar2)  
  sc22 <- betweenness(sigNet1, tar2, directed=F) 
  sc23 <- page.rank(sigNet1, algo="prpack", vids=tar2)$vector
