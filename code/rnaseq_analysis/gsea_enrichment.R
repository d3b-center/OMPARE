...
tcga_gbm_counts <- readRDS(file.path(data_dir, 'tcga', 'tcga_gbm_counts_matrix.rds'))
...
pbta_full_counts <- readRDS(file.path(data_dir, 'pbta','pbta-gene-expression-rsem-counts-collapsed.polya.stranded.rds'))
...
pnoc008_counts <- readRDS(file.path(data_dir, 'pnoc008', 'pnoc008_counts_matrix.rds'))
...
# DSigDb geneset
dsigdb_geneset <- getGmt(file.path(data_dir, 'dsigdb', 'DSigDB_All.gmt'))
...
# input data
pbta_full_tpm_melt <- pbta_full_tpm %>% rownames_to_column("gene_symbol") %>%  gather('sample', "tpm", -gene_symbol) 
tcga_gbm_tpm_melt <- tcga_gbm_tpm %>% rownames_to_column("gene_symbol") %>%  gather('sample', "tpm", -gene_symbol) 
pnoc008_tpm_melt <- pnoc008_tpm %>% rownames_to_column("gene_symbol") %>%  gather('sample', "tpm", -gene_symbol) 
pbta_full_counts_melt <- pbta_full_counts %>% rownames_to_column("gene_symbol") %>%  gather('sample', "tpm", -gene_symbol) 
tcga_gbm_counts_melt <- tcga_gbm_counts %>% rownames_to_column("gene_symbol") %>%  gather('sample', "tpm", -gene_symbol) 
pnoc008_counts_melt <- pnoc008_counts %>% rownames_to_column("gene_symbol") %>% gather('sample', "tpm", -gene_symbol) 

# if sample of interest is provided, then subset 
if(patient_of_interest != "NULL"){
  pnoc008_tpm_melt <- pnoc008_tpm_melt %>%
    filter(sample == patient_of_interest)
  pnoc008_counts_melt <- pnoc008_counts_melt %>%
    filter(sample == patient_of_interest)
}

# function to read existing gsea output, process and add current subject to it if not already present
gsea_enrichment <- function(gsea_output, exp.data.counts, exp.data.tpm, refData.counts, gene_set, comparison) {
...
gsea_output <- file.path(data_dir, 'gsea', 'pnoc008_vs_gtex_brain.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
                              exp.data.counts = x, 
...
saveRDS(pnoc008_deg, file = file.path(data_dir, 'pnoc008', "pnoc008_vs_gtex_brain_degs.rds"))

# pnoc008 vs gtex brain (dsigdb)
gsea_output <- file.path(data_dir, 'dsigdb', 'pnoc008_vs_gtex_brain.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
# pnoc008 vs pbta
gsea_output <- file.path(data_dir, 'gsea', 'pnoc008_vs_pbta.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
gsea_output <- file.path(data_dir, 'dsigdb', 'pnoc008_vs_pbta.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
# pnoc008 vs pbta hgg
gsea_output <- file.path(data_dir, 'gsea', 'pnoc008_vs_pbta_hgg.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
gsea_output <- file.path(data_dir, 'dsigdb', 'pnoc008_vs_pbta_hgg.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
# pnoc008 vs tcga gbm
gsea_output <- file.path(data_dir, 'gsea', 'pnoc008_vs_tcga_gbm.rds')
plyr::d_ply(.data = pnoc008_counts_melt, 
            .variables = "sample", 
            .fun = function(x) 
              gsea_enrichment(gsea_output = gsea_output, 
...
# pbta vs gtex brain
gsea_output <- file.path(data_dir, 'gsea', 'pbta_vs_gtex_brain.rds')
if(!file.exists(gsea_output)){
  plyr::d_ply(.data = pbta_full_counts_melt, 
              .variables = "sample", 
              .fun = function(x) 
                gsea_enrichment(gsea_output = gsea_output, 
                                exp.data.counts = x, 
                                exp.data.tpm = pbta_full_tpm_melt,
                                refData.counts = gtex_brain_counts, 
                                gene_set = gene_set, 
                                comparison = paste0("GTExBrain_", ncol(gtex_brain_counts))))
}
...
# pbta vs pbta
gsea_output <- file.path(data_dir, 'gsea', 'pbta_vs_pbta.rds')
if(!file.exists(gsea_output)){
  plyr::d_ply(.data = pbta_full_counts_melt, 
              .variables = "sample", 
              .fun = function(x) 
...
# pbta vs pbta hgg
gsea_output <- file.path(data_dir, 'gsea', 'pbta_vs_pbta_hgg.rds')
if(!file.exists(gsea_output)){
  plyr::d_ply(.data = pbta_full_counts_melt, 
              .variables = "sample", 
              .fun = function(x) 
...
gsea_output <- file.path(data_dir, 'gsea', 'tcga_gbm_vs_gtex_brain.rds')
if(!file.exists(gsea_output)){
  plyr::d_ply(.data = tcga_gbm_counts_melt, 
              .variables = "sample", 
              .fun = function(x) 
                gsea_enrichment(gsea_output = gsea_output, 
                                exp.data.counts = x, 
                                exp.data.tpm = tcga_gbm_tpm_melt,
                                refData.counts = gtex_brain_counts, 
                                gene_set = gene_set, 
                                comparison = paste0("GTExBrain_", ncol(gtex_brain_counts))))
}
...
gsea_output <- file.path(data_dir, 'gsea', 'tcga_gbm_vs_tcga_gbm.rds')
if(!file.exists(gsea_output)){
  plyr::d_ply(.data = tcga_gbm_counts_melt, 
              .variables = "sample", 
              .fun = function(x) 
